...
exports.getRejectedOrderItems = async (req, res) => {
  try {
    ...
    const { count, rows: rejectedOrderItems } = await OrderItem.findAndCountAll(
      {
        where: whereClause,
    ...
    const formattedOrderItems = rejectedOrderItems.map((orderItem) => {
      // Calculate totals
      const itemTotal = parseFloat(orderItem.price) * orderItem.quantity;
      const retipTotal = parseFloat(orderItem.retip_price) || 0;
      const grandTotal = itemTotal + retipTotal;
    ...
    res.status(200).json({
      success: true,
      message: `Found ${count} rejected order item(s)`,
      data: {
    ...
    console.error("Get rejected order items error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching rejected order items",
      error: error.message,
    });
  }
};
/* Right now im using order_status: "approved"
   because the shipment  process is not yet 
   when we set when shipment feature is added 
   we will change the status of order items to "shipped" */
exports.getShippedOrders = async (req, res) => {
  try {
    ...
    const includeClause = [
      {
        model: Product,
        as: "product",
        attributes: ["id", "user_id", "category"], // Only get product id and user_id for seller lookup
        ...(sellerId && {
          where: {
            user_id: sellerId,
          },
    ...
    // Find all rejected order items with pagination
    const { count, rows: shippedOrderItems } = await OrderItem.findAndCountAll({
      where: whereClause,
      include: includeClause,
      limit: parseInt(limit),
      offset: parseInt(offset),
    ...
    if (shippedOrderItems.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No shipped order items found",
        data: {
          orderItems: [],
          pagination: {
            currentPage: parseInt(page),
    ...
    // Format the response data
    const formattedOrderItems = shippedOrderItems.map((orderItem) => {
      // Calculate totals
      const itemTotal = parseFloat(orderItem.price) * orderItem.quantity;
      const retipTotal = parseFloat(orderItem.retip_price) || 0;
      const grandTotal = itemTotal + retipTotal;
      ...
      return {
        ...
        orderStatus: orderItem.order_status,
        paymentStatus: orderItem.payment_status,
        seller_paid: orderItem.seller_paid || false,
        platformCommission: parseFloat(orderItem.platform_commission) || 0,
    ...
    res.status(200).json({
      success: true,
      message: `Found ${count} shipped order item(s)`,
      data: {
        orderItems: formattedOrderItems,
        pagination: {
          currentPage: parseInt(page),
    ...
    console.error("Get shipped order items error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching shipped order items",
      error: error.message,
    });
  }
};
